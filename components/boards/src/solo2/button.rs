...
type Adc = lpc55_hal::peripherals::adc::Adc<init_state::Enabled>;
type Dma = lpc55_hal::peripherals::dma::Dma<init_state::Enabled>;
...
impl buttons::UserPresence for SoloThreeTouchButtons<ButtonTopPin, ButtonBotPin, ButtonMidPin>
{
...
impl buttons::Edge for SoloThreeTouchButtons<ButtonTopPin, ButtonBotPin, ButtonMidPin>
{
    fn wait_for_new_press(&mut self, but: Button) -> nb::Result<(), Infallible> {
        let result = self.button_has_edge(but, Edge::Falling);

        // Erase edge with pressed status.
        if result {
            self.button_reset_state(but, -1);
            Ok(())
        } else {
    ...
    fn wait_for_new_release(&mut self, but: Button) -> nb::Result<(), Infallible> {
        let result = self.button_has_edge(but, Edge::Rising);
        if result {
            self.button_reset_state(but, 1);
            Ok(())
        } else {
